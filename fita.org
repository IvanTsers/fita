...
>>>>>>> 674d5c0c4d582fbef1ef85b97061399fb9530c0d
...
and write the resulting subtree.
#+end_export
...
#+begin_export latex
The tracking the visited nodes is needed for traversing the tree. We
declare an array to keep IDs of the visited nodes. The array is to be
reset each time a node-visiting function is called.
#+end_export
...
the leaf Label is appended to the \texttt{leafL} slice. In case
...
The loop stops when the Id of the input node \texttt{n} is appended to
