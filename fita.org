...
#+begin_src go <<Set usage>>=
  u := "fita [tree file]... [target list file]... [option]"
  p := "Find the target clade subtree."
  e := "fita -t tree.nwk -l targets.txt -verbose"
  clio.Usage(u, p, e)
#+end_src
...
#+begin_export latex
The user sets the name of the input files (\texttt{-t} for the tree
and \texttt{-l} for the list of targets). The user can opt to use
verbose mode (\texttt{--verbose}).
#+end_export
...
\end{enumerate}
If \texttt{fita} runs in the verbose mode (\texttt{--verbose}), the
summary on intermediate results is printed out.
...
#+begin_src go <<Reset the previous results>>=
  rVotes, lVotes = 0, 0
...
WARNING: one of the children may be a terminal node...
#+end_export

#+begin_src go <<Get leaf labels>>=
  //check fo the terminal node
  if rChild.Child == nil {
...
#+begin_export latex
The function \texttt{getLeafLabels()} runs recursively across a tree
rooted on the node \texttt{n} and returns a slice of the leaf labels
of a subtree.
#+end_export

#+begin_src go <<Functions>>=
  func getLeafLabels(n *nwk.Node, l []string) []string {
	  if n == nil {return l} //else {fmt.Printf("    we are here, %s\n", n.Label)}
	  if n.Child == nil {
		  l = append(l, n.Label)
	  }
...
#+begin_src go <<--verbose: the round results>>=
  fmt.Printf(
...
and insert a parenthesis and a semicolon in the beginning end the of
the string, respectively, to designate the root properly. If the
\texttt{s} flag is used, we print the stats of the run.
#+end_export
...
#+begin_export latex
We print the summary of all voting rounds.
#+end_export

#+begin_src go <<--verbose: run summary>>=
  fmt.Printf(
	  " round: %d\tone of the clades consists of targets only\n",
	  round-1)
  fmt.Printf(
	  " round: %d\treturned to the parent node (%s)\n" +
	  "\nThe found target clade has %d of %d targets.\n",
	  round-1, cRoot.Label, winVotes, len(targetList))
  fmt.Printf("\nThe target clade:\n\n")	
#+end_src
